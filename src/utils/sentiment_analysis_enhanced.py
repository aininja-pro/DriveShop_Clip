"""
Enhanced sentiment analysis module that re-extracts YouTube content if needed
"""
import os
import json
        ...
        try:
            ...
            if 'youtube.com' in clip_data.get('clip_url', '') or 'youtu.be' in clip_data.get('clip_url', ''):
                ...
                if not content or len(content) < 500:
                    logger.info(f"YouTube clip {clip_data.get('wo_number')} has insufficient content ({len(content)} chars), re-extracting...")
            ...
            # Try to extract year and trim from model field
            year = None
            trim = None
            if model:
                # Common pattern: "2024 Model Name Trim"
                ...
                if model_parts and model_parts[0].isdigit() and len(model_parts[0]) == 4:
                    ...
                    if len(model_parts) > 2:
                        ...
                        trim_indicators = ['limited', 'sport', 'touring', 'base', 's', 'ex', 'lx', 'se', 'sel', 'xle', 'xse']
                        if potential_trim.lower() in trim_indicators or len(potential_trim) <= 4:
                            trim = potential_trim
            ...
            logger.error(f"❌ Error analyzing clip {clip_data.get('wo_number')}: {e}")
            return {
                'error': str(e),
                'sentiment_completed': False
            }
